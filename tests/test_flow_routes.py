...
class TestFlowCard:
    def test_get_card_returns_teach_or_mcq(self, client):
        ...
        assert (
            "Got it" in response.text
            or "New Concept" in response.text
            or "Back" in response.text
        )
...
class TestTranslateWordRoutes:
    def test_api_translate_word_is_accessible_without_auth(self, client):
        response = client.get(
            "/api/translate-word",
            params={"word": "hola", "context": "saludo casual"},
    ...
    def test_flow_translate_word_route_removed(self, client):
        from spanish_vibes.auth import (
            SESSION_COOKIE_NAME,
            create_user,
            create_user_session,
        )

        user = create_user("tester_route_removed", "password123")
